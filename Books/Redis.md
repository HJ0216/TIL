...
## Keys vs Scan
* keys
  * 단일 스레드 기반의 Redis에서 많은 시간을 소모하며, 그 동안 다른 명령어는 처리되지 않음
* scan
  * 동일한 작업(패턴 검색)을 비블로킹 방식으로 수행
  * 한 번에 전체 키셋을 스캔하지 않고, 여러 번 나눠서 검색 → 반복 호출을 통해 모든 데이터를 모아야 하므로, 추가적인 코드 작성 필요
  * Redis의 키 공간이 실시간으로 변경될 수 있기 때문에, 검색 도중 새로운 키가 추가되거나 삭제될 경우 정확한 결과를 보장하지 않음

```bash
SCAN 0 MATCH * COUNT 100
# CURSOR
#  0: 검색 시작, 이 외: 커서값
# MATCH *: 와일드카드, 모든 키를 검색
# COUNT 100: 100개 조회
...
# cursor: 다음 스캔 작업을 시작할 위치를 나타내는 커서 값
# keys: 리스트, 문자열 형태의 키들로 구성
```
